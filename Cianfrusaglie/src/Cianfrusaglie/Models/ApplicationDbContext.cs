   ...
   public class ApplicationDbContext : IdentityDbContext< User, IdentityRole, string> {
      ...
      public DbSet< ImageUrl > ImageUrls { get; set; }
        ...
        protected override void OnModelCreating( ModelBuilder builder ) {
         ...
         builder.Entity< AnnounceFormFieldsValues >().HasOne( pc => pc.FormField ).WithMany( p => p.AnnouncesFormFields )
            .HasForeignKey( pc => pc.FormFieldId );

         builder.Entity< AnnounceFormFieldsValues >().HasOne( pc => pc.Announce ).WithMany( c => c.AnnouncesFormFields )
            .HasForeignKey( pc => pc.AnnounceId );
         ...
         builder.Entity< Interested >().HasOne( u => u.Announce ).WithMany( u => u.Interested ).OnDelete(
            DeleteBehavior.Restrict );
         builder.Entity< Interested >().HasOne( u => u.User ).WithMany( u => u.InterestedAnnounces ).HasForeignKey( u => u.UserId ).OnDelete(
            DeleteBehavior.Restrict );
